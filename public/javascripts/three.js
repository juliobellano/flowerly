...
function main() {
  /**
   * Load flower positions from the server using a giftcard ID
   * @param {string} specificId - Optional ID to load (defaults to URL param)
   */
      ...
      data.data.flowers.forEach((flowerData) => {
        try {
          ...
          if (
            ...
            textureIndex < 0 ||
            textureIndex >= geometries.length
          ) {
  ...
  editorRenderer.setSize(
    editorCanvas.clientWidth * window.devicePixelRatio,
    editorCanvas.clientHeight * window.devicePixelRatio,
    false
  );
  editorRenderer.shadowMap.enabled = true;

  /**
   * Create a Three.js scene for flower options
   * @param {HTMLElement} sceneElement - DOM element to contain the scene
   * @returns {Object} Scene information object with scene, camera, and controls
   */
  ...
  // Set up the main editor scene with camera, lighting, and controls.
  function makeEditorScene() {
    Editorscene = new THREE.Scene();

    Editorcamera = new THREE.PerspectiveCamera(
      60,
      editorCanvas.clientWidth / editorCanvas.clientHeight,
      0.1,
      1000
    );
  ...
  /**
   * Set up all flower options in the side panel
   * @returns {Array} Array of scene objects for each flower option
   */
  function setUpOptionScene() {
    ...
    for (let i = 0; i < geometries.length; i++) {
      ...
      element.addEventListener("pointerdown", (event) => {
        ...
        const arrayNumber =
          event.currentTarget.getAttribute("data-array-number");
      ...
      // Create scene
      const flowerPreviewScene = makeOptionScene(sceneElement);
      ...
      const mesh = new THREE.Mesh(geometry, material);
      flowerPreviewScene.scene.add(mesh);

      scenes.push(flowerPreviewScene);
    }
  ...
  function setUpEditorScene() {
    ...
    const textureLoader = new THREE.TextureLoader();
    const Bouquettexture1 = new THREE.TextureLoader().load(
      "textures/lowerLayer.PNG"
    );
    const Bouquettexture2 = new THREE.TextureLoader().load(
      "textures/upperLayer.PNG"
    );
    ...
    const materials = {
      ...
      bouquetLower: new THREE.MeshBasicMaterial({
        map: Bouquettexture1,
        ...commonMaterialProps,
      }),
      bouquetUpper: new THREE.MeshBasicMaterial({
        map: Bouquettexture2,
        ...commonMaterialProps,
      }),
  ...
  /**
   * Add a flower to the editor canvas
   * @param {number} textureIndex - Index of the flower texture to use
   */
  function putOnCanvas(arr) {
    ...
    setTimeout(() => {
      mesh.position.set(0, 5, maxZ + 0.1);
    }, 200);
  ...
  const onPointerMove = (event) => {
    ...
    const targetObjects = Editorscene.children.filter(
      (obj) => obj !== bouquetUpper && obj !== ribbon
    );
    const hits = raycaster.intersectObjects(targetObjects, true);

    const flowerHits = hits.filter(
      (hit) => hit.object !== ground && hit.object !== bouquetLower
    );
  ...
  function animate() {
    ...
    const optionPanel = document.getElementById("divOption");
    const scrollTop = optionPanel.scrollTop;
    ...
    // Render each scene
    scenes.forEach((flowerPreviewScene) => {
      const { scene, camera, controls, element } = flowerPreviewScene;
...
/**
 * Save the current flower arrangement to the server
...
document
  ...
  .addEventListener("click", saveFlowerPositions);
document.getElementById("loadButton").addEventListener("click", Hearts);

// Display a conffeti effect
